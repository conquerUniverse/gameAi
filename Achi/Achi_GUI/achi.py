...
#     return {'position': pos, 'prize': best}
  ...
  def playerMove(self, fromSquare, toSquare):
    if self.GameOver():
        return -2
    a, b = fromSquare, toSquare
    if(self.count>5):
        ...
        if self.state[a//self.size][ (a)%self.size]!=0 or fromSquare is not toSquare:
            return -1
        self.state[a//self.size][ (a)%self.size] = 2
